        ...
        public static List<JpegFragment> SplitFragments([NotNull] SequentialReader reader)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        var dummy_segment = new JpegSegment(segmentType, reader.GetBytes(0), reader.Position);
                        // To read the segment payload, the segment length must be decoded:
                        var payloadLength = dummy_segment.DecodePayloadLength(reader.GetByte(), reader.GetByte());
                        if (payloadLength < 0)
