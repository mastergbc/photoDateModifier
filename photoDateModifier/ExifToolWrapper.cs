...
namespace SubWapper
{
    ...
    public sealed class ExifToolWrapper : IDisposable
    {
        ...
        private const string ExeName = "Tool\\exiftool.exe";
        private const string Arguments = "-a -G1 -s -j -charset UTF-8";
        ...
        public ExifToolWrapper(string imagePath, string exePath = null)
        {
            ...
            if(!File.Exists(ExifToolPath))
                throw new Exception($"{ExeName} not found");

            _psi = new ProcessStartInfo
            {
                FileName = ExifToolPath,
                Arguments = Arguments + " " + imagePath,
                CreateNoWindow = true,
        ...
        public string Start()
        {
            ...
            try
            {
                ...
                if (Status != ExeStatus.Stopped)
                    throw new Exception("Process is not stopped");
                ...
                jsonResult = _proc.StandardOutput.ReadToEnd();

                _proc.WaitForExit();
            }
            catch (Exception excep)
            {
                MessageBox.Show($"Error LoadCurrImage[{_proc.ExitCode}]: {excep.Message}");
            }
            return jsonResult;
        ...
        public void Stop()
        {
            ...
            if(Status != ExeStatus.Ready)
                throw new Exception("Process must be ready");
